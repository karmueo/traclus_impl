...
@author: Alex
'''
from traclus_impl.distance_functions import perpendicular_distance, angular_distance2, parrallel_distance, angular_distance, perpendicular_distance2, angular_distance3, velocity_distance
from traclus_impl.generic_dbscan import Cluster, ClusterCandidate, ClusterFactory, ClusterCandidateIndex
...
class TrajectoryLineSegment(ClusterCandidate):
    ...
    def distance_to_candidate(self, other_candidate):
        ...
        res_v = perpendicular_distance(self.line_segment, other_candidate.line_segment)
        res_a = angular_distance(self.line_segment, other_candidate.line_segment)
        # res_a = angular_distance2(self.line_segment, other_candidate.line_segment)
        res_h = parrallel_distance(self.line_segment, other_candidate.line_segment)
        # res_s = velocity_distance(self.line_segment, other_candidate.line_segment)

        tot = res_v + res_a + res_h
...
class TrajectoryLineSegmentCandidateIndex(ClusterCandidateIndex):
    ...
    def my_find_neighbors_of(self, cluster_candidate):
        neighbors = ClusterCandidateIndex.find_neighbors_of(self, cluster_candidate)
        return neighbors
